...
class DQNAgent:
    ...
    def build_model(self):
        model = Sequential()
        model.add(
            Conv2D(32, (8, 8), strides=(4, 4), padding="same", input_shape=(img_rows, img_cols, img_channels))
        )  # 80*80*4
...
if __name__ == "__main__":
    ...
    action_size = env.action_space # Steering and Throttle

    agent = DQNAgent(state_size, action_size, train=not args.test)
    ...
    if not agent.train:
        ...
        agent.load_model("{:s}".format(args.model))
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                if agent.train:
                    agent.save_model("{:s}".format(args.model))
